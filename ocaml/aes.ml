...
(*
* Table de substitution réciproque
*)
...
(*
* Subtitution inverse
* On remplace chaque élément du tableau par son antécédent dans la subtitution box,
* c'est à dire l'image par la subtitution box inverse
*)
let substitution_inverse entree = Array.map (fun x -> rsbox.(x)) entree
...
* On permutte simplement l'emplacement des éléments dans le tableau
*)
let decalage_inverse entree =
    [|
        (* 0 -> *)      (* 1 -> *)      (* 2 -> *)      (* 3 -> *)
        entree.(0);     entree.(13);    entree.(10);    entree.(7); 
        entree.(4);     entree.(1);     entree.(14);    entree.(11);
        entree.(8);     entree.(5);     entree.(2);     entree.(15);
        entree.(12);    entree.(9);     entree.(6);     entree.(3)
    |]
...
(*
* On génère le tableau de clés
*)
let tableau_clefs cle =
    (* On définit le tableau *)
    let cles = Array.make 11 cle in
    for k = 1 to 10 do
        cles.(k) <- clef_suivante cles.(k-1) k
    done;
    cles
...
(*
* Tour inverse
* On combine les étapes inverses
*)
...
let chiffrer entree cle =
    (* On calcul toutes les clefs *)
    let clefs = tableau_clefs cle in
...
* Déchiffrement
*)
let dechiffrer entree cle =
    (* On calcul toutes les clefs *)
    let clefs = tableau_clefs cle in

    (* On lance les tours *)
    let toured = tour_inverse entree clefs 10 in
